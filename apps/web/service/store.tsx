...
export const useCartStore = create<CartState>((set, get) => ({
      ...
      if (exists) {
        ...
        return {
          items: state.items.map((i) =>
            i.id === item.id ? { ...i, quantity: clampedQty } : i
          ),
        };
      }
  ...
  updateQty: (id, quantity) =>
    ...
    items: state.items.map((i) => {
      ...
      const clamped = typeof max === "number" ? Math.min(Math.max(1, quantity), max) : Math.max(1, quantity);
  ...
  syncWithServer: async () => {
    ...
    try {
      ...
      const data = (await res.json()) as {
        items: Array<{ id: string; name: string; image: string; price: number; quantity: number; maxQuantity: number }>;
